				...
				if($query){
					if(isset($_POST['tags']) && strlen($_POST['tags']) > 0){
						...
						foreach($tags as $tag){
							...
							$sttag = $conn->prepare('SELECT * FROM tag WHERE nama LIKE :nama');
							$sttag->execute([
								':nama'=>'%'.$tag.'%'
							]);

							$mtag = $sttag->fetch(PDO::FETCH_OBJ);
							$tag_id = null;
							if(!$mtag){
								$mtag = $conn->prepare('INSERT INTO tag (nama) VALUES (:nama)');
								$mtag->execute([
									':nama'=>$tag
								]);		
							...
							$ftag = $conn->prepare('INSERT INTO file_tag (file_id, tag_id) VALUES (:id, :tid)');
							$ftag->execute([
								':id'=>$file_id,
								':tid'=>$tag_id
							]);		
	...
	<div class="panel-body" id="files">
		<form class="form-horizontal" method="POST" action="upload.php?folder=<?= $folder ?>" enctype="multipart/form-data">
		  ...
		  <div class="form-group">
			...
			<div class="col-sm-10">
			  <input type="text" name="tags" class="form-control" placeholder="Tags Pisahkan dengan koma">
			</div>
		  </div>
		  <div class="form-group">
